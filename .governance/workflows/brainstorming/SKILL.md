...
2. **Check references** — 查看 `.governance/references/` 是否有相关的业务/技术参考资料，有则在方案中引用
...
digraph brainstorming {
    ...
    "Explore project context" -> "Check references";
    "Check references" -> "Visual questions ahead?";
...
- **Check references:** 查看 `.governance/references/` 是否有相关的业务/技术参考资料
  - 如果有，在提出方案时引用："根据《xxx》的分析..."
  - 如果没有或不够，可用 web search 补充，或向用户询问
  - 如果发现有价值的外部资料，主动建议用户添加到 references
...
- Write the validated design (spec) to `.governance/specs/NNN-<feature-name>/spec.md`
  - NNN is the next sequential number (scan existing specs to determine)
...
> **Note:** The visual companion requires a browser-based server component that may not be available in all environments. If unavailable, proceed with text-only brainstorming.